...
contract GitViction is AragonApp {
    ...
    function stakeToProposal(uint256 id, uint256 amount) external {
        ...
        require(stakes_per_voter[msg.sender] + amount < token.balanceOf(msg.sender), "stake fail");
    ...
    function withdrawFromProposal(uint256 id, uint256 amount) external {
        require(proposals[id].stakes_per_voter[msg.sender] >= amount, "withdraw fail");
        uint256 old_staked = proposals[id].staked_tokens;
        proposals[id].staked_tokens -= amount;
        stakes_per_voter[msg.sender] -= amount;
        calculateAndSetConviction(id, old_staked);
    ...
    function calculateConviction(uint256 time_passed, uint256 last_conv, uint256 old_amount, uint256 new_amount) 
    ...
    function calculateThreshold(uint256 amount_commons) public view returns (uint256 threshold) {
        return 400 * (10 ** 18);
    }
